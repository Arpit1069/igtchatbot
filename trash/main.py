...
def load_data():
    ...
    global df
    try:
        df = pd.read_csv("supermarket_sales_m.csv")
...
def create_agent(model_choice, session_id):
    """Create the pandas dataframe agent for a session"""
    global df, agents
    
    if df is None:
        return None, "Data not loaded. Please load data first."
    ...
    try:
        ...
        return agent, f"{model_choice} model initialized successfully"
    except Exception as e:
...
def get_system_prompt():
    """Return a simplified system prompt for the agent."""
    return (
        "You are a helpful data analyst assistant. "
        "Answer user questions about the supermarket sales data clearly and accurately. "
        "When providing numerical answers, give the exact number followed by a brief explanation if needed. "
        "Use the pandas dataframe 'df' to analyze the data and provide insights. "
        "Format your answers in a friendly, conversational tone."
    )
...
def api_load_data():
    ...
    if success:
        # Clear existing agents when new data is loaded
        global agents
        agents.clear()
